import XCTest
@testable import PlaydatePlanner

@MainActor
final class PlaydatePlannerTests: XCTestCase {
    ...
    func testFreeLimitBlocksAddWhenReached() {
        ...
        for _ in 0..<(Store.freeLimit + 5) {
            _ = store.add(Playdate())
        }
        let result = store.add(Playdate())
